...
class BehaviorDataController extends Controller
{
    ...
    public function getBehaviorData(Request $request)
    {
        try {
            ...
            foreach ($monthsToProcess as $month) {
                ...
                // Build base query for male violations - join with users table to get gender
                $maleQuery = Violation::join('student_details', 'violations.student_id', '=', 'student_details.student_id')
                    ->join('pnph_users', 'student_details.user_id', '=', 'pnph_users.user_id')
                    ->where('pnph_users.gender', 'M')
                    ->whereMonth('violations.violation_date', $monthNum)
                    ->whereYear('violations.violation_date', $currentYear)
                    ;

                // Apply batch filter if specified
                if ($batchFilter !== 'all') {
                    $maleQuery->where('violations.student_id', 'like', $batchFilter . '%');
                }

                $maleCount = $maleQuery->count();

                // Build base query for female violations - join with users table to get gender
                $femaleQuery = Violation::join('student_details', 'violations.student_id', '=', 'student_details.student_id')
                    ->join('pnph_users', 'student_details.user_id', '=', 'pnph_users.user_id')
                ...
                // Apply batch filter if specified
                if ($batchFilter !== 'all') {
                    $femaleQuery->where('violations.student_id', 'like', $batchFilter . '%');
                }
    ...
    private function getWeeklyViolationData($monthNum, $year, $gender, $batchFilter = 'all')
    {
        ...
        // Get all violations for this month and gender - join with users table
        $genderValue = (strtolower($gender) === 'male') ? 'M' : 'F';

        $violationsQuery = Violation::join('student_details', 'violations.student_id', '=', 'student_details.student_id')
            ->join('pnph_users', 'student_details.user_id', '=', 'pnph_users.user_id')
            ->where('pnph_users.gender', $genderValue)
            ->whereMonth('violations.violation_date', $monthNum)
            ->whereYear('violations.violation_date', $year)
            ;

        // Apply batch filter if specified
        if ($batchFilter !== 'all') {
            $violationsQuery->where('violations.student_id', 'like', $batchFilter . '%');
        }
        ...
        if (!empty($studentIds)) {
            ...
            $studentsCollection = \App\Models\User::join('student_details', 'pnph_users.user_id', '=', 'student_details.user_id')
        ...
        foreach ($violations as $violation) {
            if ($violation->student_id && isset($students[$violation->student_id])) {
                ...
                $violation->first_name = $student->user_fname;
                $violation->last_name = $student->user_lname;
            }
        }
        ...
        foreach ($violations as $violation) {
            ...
            if ($weekIndex >= 0 && $weekIndex < $numWeeks) {
                ...
                // If not, try to get the student information from the database
                    ...
                    try {
                        $studentDetails = \App\Models\StudentDetails::where('student_id', $violation->student_id)->first();
                        if ($studentDetails && $studentDetails->user) {
                            $studentName = $studentDetails->user->user_fname . ' ' . $studentDetails->user->user_lname;
                        }
                    } catch (\Exception $e) {
