        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterTable() {
                ...
                rows.forEach(row => {
                    ...
                    // Check if student ID matches the selected batch (first 4 characters)
                    let matchesBatch = (batch === 'all') || (studentId.substring(0, 4) === batch);
                ...
                // Optional: Show count of visible students
...
<th>Action</th>
            ...
            <tbody id="studentsList">
                ...
                <tr>
                    <td>{{ $student->name }}</td>
                    <td>{{ $student->studentDetails->student_id ?? 'N/A' }}</td>
                    <td>{{ $student->gender === 'M' ? 'Male' : ($student->gender === 'F' ? 'Female' : 'Not specified') }}</td>
                    <td>
                        @php
                            $studentIdForViolations = $student->studentDetails->student_id ?? null;
                            $violationCount = $studentIdForViolations ? \App\Models\Violation::where('student_id', $studentIdForViolations)->count() : 0;
                        @endphp
                    ...
                    <td>
                        <a href="{{ url('/educator/student-profile/' . ($student->studentDetails->student_id ?? $student->user_id)) }}" class="btn btn-info btn-sm">
