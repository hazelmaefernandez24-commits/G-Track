...
<div class="container-fluid" style="padding: 20px;">
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px;">
        <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <h1 style="color: #333; font-weight: 600; margin: 0; font-size: 24px;">Edit Event</h1>
        </div>
    ...
    <div style="background: white; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); padding: 30px; max-width: 800px;">
        <form method="POST" action="{{ route('training.calendar.update', $event) }}">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                ...
                <div>
                    ...
                    <select id="category" 
                        ...
                        <option value="school_activity" {{ old('category', $event->category) == 'school_activity' ? 'selected' : '' }}>School Activity</option>
                        <option value="holiday" {{ old('category', $event->category) == 'holiday' ? 'selected' : '' }}>Holiday</option>
                        <option value="examination" {{ old('category', $event->category) == 'examination' ? 'selected' : '' }}>Examination</option>
                        <option value="deadline" {{ old('category', $event->category) == 'deadline' ? 'selected' : '' }}>Deadline</option>
                        <option value="vacation" {{ old('category', $event->category) == 'vacation' ? 'selected' : '' }}>Vacation</option>
                        <option value="special" {{ old('category', $event->category) == 'special' ? 'selected' : '' }}>Special Event</option>
                    </select>
                    @error('category')
                        <div style="color: #e74c3c; font-size: 0.9em; margin-top: 5px;">{{ $message }}</div>
                    @enderror
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
                <div>
                    ...
                    <select id="semester" 
                        ...
                        <option value="first" {{ old('semester', $event->semester) == 'first' ? 'selected' : '' }}>First Semester</option>
                        <option value="second" {{ old('semester', $event->semester) == 'second' ? 'selected' : '' }}>Second Semester</option>
                        <option value="summer" {{ old('semester', $event->semester) == 'summer' ? 'selected' : '' }}>Summer</option>
                    </select>
                    @error('semester')
                        <div style="color: #e74c3c; font-size: 0.9em; margin-top: 5px;">{{ $message }}</div>
                    @enderror
            ...
            <div style="display: flex; gap: 15px; justify-content: flex-end;">
                ...
                <button type="submit" 
                        style="background-color: #ffc107; color: #212529; padding: 12px 24px; border: none; border-radius: 5px; font-weight: 500; cursor: pointer; transition: all 0.3s ease;">
